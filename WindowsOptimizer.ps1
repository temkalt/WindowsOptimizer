...
$Host.UI.RawUI.WindowTitle = "WindowsOptimizer 2.0 - Ultimate Esports Kernel & System Suite"
...
function Run-DeepAudit {
    ...
    Write-Host "============================================================================" -ForegroundColor Cyan
    Write-Host " [*] Сканирование ядра Windows, реестра, таймеров, драйверов и служб...`n" -ForegroundColor DarkGray
    ...
    foreach ($item in $checks.GetEnumerator()) {
        if ($item.Value) {
            ...
            Write-Host "  [-] $($item.Key)" -ForegroundColor DarkGray
        }
    }
...
function Apply-EsportsMaximum {
    ...
    # Call backend preset
    try {
        $res = Invoke-RestMethod -Uri "http://localhost:5050/api/presets/ultimate-cybersport" -Method Post -TimeoutSec 5 -ErrorAction SilentlyContinue
    } catch {}
    ...
    Write-Host "`n[УСПЕХ] ВСЕ КИБЕРСПОРТИВНЫЕ ПАРАМЕТРЫ ЯДРА УСПЕШНО ПРИМЕНЕНЫ!" -ForegroundColor Cyan
...
function Show-CategoriesMenu {
    $categories = @(
        ...
        "11. Устройства и MSI Mode (GPU & NIC Message Signaled Interrupts)",
...
function Show-CategoryFiles {
    ...
    while ($true) {
        ...
        if ($fChoice.ToUpper() -eq "A") {
            Write-Host "`n[*] Пакетное применение раздела..." -ForegroundColor Yellow
            foreach ($f in $files) {
                Execute-TweakFile $f.FullName
            }
            Write-Host "`n[+] Все файлы раздела успешно применены!" -ForegroundColor Green
...
while ($true) {
    ...
    Write-Host "  [2] 🛡️ БЕЗОПАСНЫЙ ИГРОВОЙ РЕЖИМ (Без Риска)" -ForegroundColor Green
    ...
    Write-Host "  [7] 🖥️ ЗАПУСТИТЬ ГРАФИЧЕСКОЕ ПРИЛОЖЕНИЕ (GUI)" -ForegroundColor Magenta
    ...
    switch ($choice) {
        ...
        "0" {
            Write-Host "`nВыход из WindowsOptimizer. До свидания!" -ForegroundColor Green
            exit
        }
    }
}
